...
class Ball():
    def __init__(self):
        ...
        self.x = random.randrange(0, 700 - self.diameter)
        self.y = random.randrange(0, 500 - self.diameter)
    ...
    def draw(self):
        ...
        pygame.draw.ellipse(screen, BLACK, [self.x, self.y, self.diameter, self.diameter], 5)
    def move(self):
        self.x += self.xspeed
        self.y += self.yspeed
        if self.x > 700 - self.diameter:
            self.xspeed *= -1
        if self.x < 0:
            self.xspeed *= -1
        if self.y > 500 - self.diameter:
            self.yspeed *= -1
        if self.y < 0:
            self.yspeed *= -1
class Bubble(Ball):
    def __init__(self):
        ...
        self.xspeed = 0
        self.yspeed = random.random() * 3 + 0.5
...
class Rectangle():
    ...
    def draw(self):
        ...
        pygame.draw.rect(screen, BLACK, [self.x, self.y, self.height, self.width], 3)
    def move(self):
        self.x += self.xspeed
        self.y += self.yspeed
...
while not done:
    ...
    if pygame.mouse.get_pressed()[0]:
        new_bubble = Bubble()
        new_bubble.x = x - new_bubble.diameter / 2
        new_bubble.y = y - new_bubble.diameter / 2
