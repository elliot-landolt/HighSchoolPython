...
class Scoreboard():
    def __init__(self):
        ...
        self.x = 550
        self.y = BANNER_GAP
        self.font = pygame.font.Font('COURIER.ttf', 18)
        self.font_big = pygame.font.Font('COURIER.ttf', 40)
        self.font_small = pygame.font.Font('COURIER.ttf', 12)
        self.score_font = pygame.font.Font('COURIER.ttf', 45)
    def draw_score(self, screen, score, high, time):
        ...
        pygame.draw.rect(screen, WHITE, [575, 50, 800, 100])
        pygame.draw.rect(screen, BLACK, [575, 50, 800, 100], 2)
        screen.blit(score_text, [600, 60])
        screen.blit(high_text, [600, 105])
        if time < 10:
        ...
        screen.blit(time_text, [1000, 105])           
    def draw(self, screen):
        ...
        for i in range(len(self.correct_list_score)):
            if len(self.correct_list_score[i]) > 1:
                for j in range(1,len(self.correct_list_score[i])):
                    ...
                    screen.blit(text, [self.x + i * 100 + i * i * 7, self.y + j * 25])
    def make_randolist(self):
        ...
        for i in range(len(self.possible_list)):
            for j in range(1, len(self.possible_list[i])):
                if self.possible_list[i][j] in self.correct_list[i]:
                    self.reveal_list_correct.append([self.possible_list[i][j], [random.randrange(-20, SCREEN_WIDTH), random.randrange(-20, SCREEN_HEIGHT)], BLACK])
                else:
                    g = random.randrange(50,255)
                    b = random.randrange(50,255)
                    r = random.randrange(50,255)
                    self.reveal_list.append([self.possible_list[i][j], [random.randrange(-20, SCREEN_WIDTH), random.randrange(-20, SCREEN_HEIGHT)],(r,g,b) ])
...
class Inputbox():
    ...
    def __init__(self):
        ...
        self.x = EDGE_GAP
        self.y = 50
        self.font = pygame.font.Font('COURIER.ttf', 75)
        self.font_points = pygame.font.SysFont('Calibri', 30, False, False)

    def draw(self, screen):
        pygame.draw.rect(screen, BLACK, [self.x, self.y, 460, 100])
        pygame.draw.rect(screen, WHITE, [self.x + 2, self.y + 2, 456, 96])
        text = self.font.render(self.text, True, BLACK)
        screen.blit(text, [self.x + 10, self.y + 17])
    def draw_wordscore(self, screen, scrabble_points):
        ...
        screen.blit(text, [self.x + 10, self.y + 110])   
        screen.blit(text2, [self.x + 10, self.y + 130])
        screen.blit(text3, [self.x + 10, self.y + 160])
        pygame.draw.line(screen, BLACK, [self.x + 10, self.y + 150], [self.x + 450, self.y + 150], 3)
...
class Letterbox(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self.rect = self.image.get_rect()
        self.font_bold = pygame.font.Font('COURIER.ttf', 77)
        self.font = pygame.font.Font('COURIER.ttf', 80)
        self.font_small = pygame.font.Font('COURIER.ttf', 35)
    ...
    def draw_letter(self,screen, letter_list, scrabble_points):
        ...
        screen.blit(text, [self.rect.x + 23, self.rect.y + 18])
        ...
        screen.blit(value, [self.rect.x + 68, self.rect.y + 68])
...
class Game(object):
    ...
    def display_frame(self, screen):
        ...
        if self.game_over:
            ...
            font = pygame.font.Font("COURIER.ttf", 60)
            ...
            text = font.render("Welcome to Word Jumble!  Create any word (4 letters or more) from the tiles provided.", True, BLACK)
            text2 = font.render("Long words receive a score multiplier.  Big word scores receive a time bonus.", True, BLACK)
            text3 = font.render("Press any key to begin.  GOOD LUCK! (Aaron Lee, 2018)", True, BLACK)
            center_x = (SCREEN_WIDTH // 2) - (text.get_width() // 2)
            center_y = (SCREEN_HEIGHT // 2) - (text.get_height() // 2)
            screen.blit(text, [center_x, center_y - 50])
            center_x = (SCREEN_WIDTH // 2) - (text2.get_width() // 2)
